...
the pip-installed `ultramemory` package (`pip install ultramemory-mcp`).
...
def _cmd_enable(args: argparse.Namespace) -> int:
    ...
    if UltraMemoryProvider is None:
        print("error: run `enable` from the installed package (pip install ultramemory-mcp).", file=sys.stderr)
        return 2
